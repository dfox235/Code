...
void generation_aleatoire(Terrain *T, int l, int h, float dObst) {
  ...
  // while (chemin<1){
    for (int i=0; i<nb;i++){
      ...
      d=rand() % h; //variable aléatoire de la hauteur
      c=rand() % l; //variable aléatoire de la largeur
      int f=rand(); //variable qui placera soit EAU soit ROCHER en fonction de si f est pair ou pas
      if ((T->tab[c][d]!=LIBRE) || ((c==y)&&(d==x))){
        while ((T->tab[c][d]!=LIBRE) || ((c==y)&&(d==x))){
          c=rand(); //variable aléatoire de la hauteur
          c=c%h;
          d=rand(); //variable aléatoire de la largeur
          d=d%l;
        }
      }
      if (f%2==1){
  ...
  //   chemin=existe_chemin_vers_sortie(T);
  // }
}
